...
export abstract class AbstractDomainEntity {
    ...
    /**
     * @deprecated please use AbstractDomainModel.from() instead
     */
    constructor() {
      // const propAliases: any = Reflect.getMetadata('propAliases', this);
      //
      // forEach<string>(Reflect.getMetadata('modelProps', this), (propName) => {
      //   /* eslint-disable:next-line */
      //   this[propName] = json[propAliases[propName]] ?? json[propName];
      // });
    }
  
    /**
     * Serializes this AbstractDomainModel to json. mapping back to prop aliases defined
     * in @Prop(<prop-alias>) and removing any properties decorated with @JsonIgnore().
